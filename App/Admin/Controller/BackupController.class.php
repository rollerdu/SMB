...
class BackupController extends BaseController
{
    ...
    {
        parent::_initialize();
        $this->backup_path = '/Attachment/backup/';
    ...
    public function index()
    {
        ...
        $this->assign('tables', D()->db()->getTables()); //显示所有数据表
        $this->display();
    }

    public function index_post(){
        if (IS_POST || isset($_GET['dosubmit'])){
            $sizelimit = abs(intval(I("param.sizelimit"))) ? abs(intval(I("param.sizelimit"))) : $this->error('请输入每个分卷文件大小');
            $this->backup_name = isset($_POST['backup_name']) && trim($_POST['backup_name']) ? trim($_POST['backup_name']) : $this->error('请输入备份名称');
            if ($_POST['backup_type'] == 'full'){
                $backup_tables = array_flip(D()->db()->getTables());
                foreach($backup_tables as $k =>$val){
                    $backup_tables[$k] = -1;
                }
            }else {
...
//            mkdir(SITE_URL . $this->backup_path . $this->backup_name);
            mkdir('./'. $this->backup_path . $this->backup_name);
...
//        var_dump($this->_get_backups());
    ...
    public function del_backup()
    {
        ...
        $load = './Attachment/backup/'.$_GET['backup'];
        $tpl = dir($load);
